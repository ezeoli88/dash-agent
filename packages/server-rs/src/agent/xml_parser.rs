//! XML fallback parser for MiniMax model output.
//!
//! Handles `<think>` blocks (reasoning) and `<minimax:tool_call>` blocks
//! that the model sometimes emits instead of structured JSON tool calls.
