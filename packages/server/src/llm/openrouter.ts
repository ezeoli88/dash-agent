 ...
 * Uses the OpenAI SDK pointed at the OpenRouter API endpoint.
 */
export class OpenRouterProvider implements LLMProvider {
  ...
  constructor(apiKey: string, model: string) {
    ...
    this.client = new OpenAI({
      ...
      maxRetries: 2,
      defaultHeaders: {
        'HTTP-Referer': 'https://agent-board.local',
        'X-Title': 'Agent Board',
      },
    });
    this.model = model;

    logger.info('OpenRouter provider initialized', { model: this.model });
  }
  ...
  /**
   * Sends a chat request to OpenRouter with optional tool support.
   */
  async chat(messages: Message[], tools?: Tool[], options?: ChatOptions): Promise<Message> {
    ...
    try {
      ...
      if (!completion?.choices?.length) {
        logger.warn('OpenRouter returned empty or missing choices', {
          model: this.model,
          completion: JSON.stringify(completion).substring(0, 500),
        });
        throw new Error('No response choices returned from OpenRouter. The model may not support tool calling or returned an empty response.');
      }
      ...
      // Log non-API errors too for debugging
      const message = error instanceof Error ? error.message : String(error);
      logger.error('OpenRouter chat error', { model: this.model, error: message });
      throw error;
    }
  }
}
