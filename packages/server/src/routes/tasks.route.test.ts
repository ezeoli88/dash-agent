...
describe("tasks routes", () => {
  beforeEach(() => {
    ...
    mocks.taskService.update.mockImplementation(
  ...
  it("blocks execute for invalid status transition", async () => {
    ...
    const response = await request(app)
      .post(`/api/tasks/${TEST_TASK_ID}/execute`)
      .send({});

    expect(response.status).toBe(400);
    expect(response.body.error).toBe("Invalid task status");
    expect(response.body.message).toContain(
      "Cannot execute task with status: draft",
    );
  });
  ...
  it("streams terminal complete event for done tasks in logs endpoint", async () => {
    ...
    expect(mocks.sseEmitter.addClient).toHaveBeenCalledWith(
      TEST_TASK_ID,
      expect.anything(),
    );
  });
  ...
  describe("POST /tasks/:id/generate-spec (deprecated)", () => {
    it("returns 410 Gone with deprecation message", async () => {
      ...
      expect(response.body.message).toContain("ya no está activo");
      expect(response.body.alternative).toBe("/tasks/:id/start");
      expect(mocks.generateSpec).not.toHaveBeenCalled();
    });

    it("returns 410 regardless of task status", async () => {
      const app = createApp();
  ...
  describe("POST /tasks/:id/regenerate-spec (deprecated)", () => {
    it("returns 410 Gone with deprecation message", async () => {
      const app = createApp();
      mocks.taskService.getById.mockReturnValue(buildTask("pending_approval"));

      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/regenerate-spec`)
        .send({});
  ...
  describe("PATCH /tasks/:id/spec (deprecated)", () => {
    it("returns 410 Gone with deprecation message", async () => {
      const app = createApp();
      mocks.taskService.getById.mockReturnValue(buildTask("pending_approval"));
  ...
  describe("POST /tasks/:id/approve-spec (deprecated)", () => {
    it("returns 410 Gone with deprecation message", async () => {
      const app = createApp();
      mocks.taskService.getById.mockReturnValue(buildTask("pending_approval"));

      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/approve-spec`)
        .send({});
  ...
  describe("POST /tasks/:id/resolve-conflicts", () => {
    it("rejects if status is NOT merge_conflicts", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/resolve-conflicts`)
        .send({});

      expect(response.status).toBe(400);
      expect(response.body.error).toBe("Invalid task status");
      expect(response.body.message).toContain(
        "Cannot resolve conflicts for task with status: coding",
      );
    });
    ...
    it("succeeds when all conflicts are resolved", async () => {
      ...
      expect(mocks.execGitOrThrow).toHaveBeenCalledWith(
        ["add", "."],
        "/tmp/worktree",
      );
      expect(mocks.execGitOrThrow).toHaveBeenCalledWith(
      ...
      expect(mocks.agentService.pushAndCreatePR).toHaveBeenCalledWith(
        TEST_TASK_ID,
      );
    });
    ...
    it("falls back to git for conflict files when task has no conflict_files", async () => {
      ...
      expect(response.status).toBe(200);
      expect(mocks.gitService.getConflictingFiles).toHaveBeenCalledWith(
        "/tmp/worktree",
      );
    });
  });
  ...
  describe("POST /tasks/:id/open-editor", () => {
    it("rejects if status is NOT merge_conflicts", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/open-editor`)
        .send({});

      expect(response.status).toBe(400);
      expect(response.body.error).toBe("Invalid task status");
      expect(response.body.message).toContain(
        "Cannot open editor for task with status: coding",
      );
    });
  ...
  describe("POST /tasks/:id/approve (PR)", () => {
    it("rejects wrong status", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/approve`)
        .send({});

      expect(response.status).toBe(400);
      expect(response.body.error).toBe("Invalid task status");
      expect(response.body.message).toContain(
        "Cannot approve task with status: coding",
      );
    });
    ...
    it("returns 400 with typed error when local repo has no origin", async () => {
      ...
      mocks.agentService.approveAndCreatePR.mockRejectedValue(
        new Error(
          'No se pudo crear PR porque el repositorio local (C:/repo) no tiene un remote "origin". Configura origin a GitHub o GitLab y reintenta.',
        ),
      );
  ...
  describe("POST /tasks/:id/request-changes", () => {
    it("rejects wrong status", async () => {
      ...
      expect(response.body.error).toBe("Invalid task status");
      expect(response.body.message).toContain(
        "Cannot request changes for task with status: draft",
      );
    });
  ...
  describe("POST /tasks/:id/pr-merged", () => {
    it("rejects wrong status", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/pr-merged`)
        .send({});
    ...
    it("succeeds for pr_created status", async () => {
      ...
      expect(mocks.agentService.markPRMerged).toHaveBeenCalledWith(
        TEST_TASK_ID,
      );
    });
  ...
  describe("POST /tasks/:id/pr-closed", () => {
    it("rejects wrong status", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/pr-closed`)
        .send({});
    ...
    it("succeeds for pr_created status", async () => {
      ...
      expect(mocks.agentService.markPRClosed).toHaveBeenCalledWith(
        TEST_TASK_ID,
      );
    });
  ...
  describe("POST /tasks/:id/approve-plan", () => {
    it("rejects if status is NOT plan_review", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/approve-plan`)
        .send({});

      expect(response.status).toBe(400);
      expect(response.body.error).toBe("Invalid task status");
      expect(response.body.message).toContain(
        "Cannot approve plan for task with status: coding",
      );
    });
  ...
  describe("POST /tasks/:id/extend", () => {
    it("returns 400 when no agent is running", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/extend`)
        .send({});
  ...
  describe("GET /tasks/:id/changes", () => {
    it("returns live worktree changes when available", async () => {
      ...
      mocks.gitService.getDiff.mockResolvedValue(
        "diff --git a/src/app.ts b/src/app.ts\n...",
      );

      const response = await request(app).get(
        `/api/tasks/${TEST_TASK_ID}/changes`,
      );
  ...
  describe("POST /tasks/:id/start", () => {
    it("succeeds for draft status", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/start`)
        .send({});
    ...
    it("rejects for non-draft/non-failed status", async () => {
      ...
      expect(response.body.message).toContain(
        "Cannot start task with status: coding",
      );
    });
  });
  ...
  describe("POST /tasks/:id/cancel", () => {
    it("cancels PM Agent for refining status", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/cancel`)
        .send({});
  ...
  describe("GET /tasks/:id/pr-comments", () => {
    it("returns 400 if no pr_url on task", async () => {
      ...
      const response = await request(app).get(
        `/api/tasks/${TEST_TASK_ID}/pr-comments`,
      );
    ...
    it("succeeds when pr_url exists and returns comments", async () => {
      ...
      expect(mocks.githubClient.getPullRequestComments).toHaveBeenCalledWith(
  ...
  describe("POST /tasks/:id/cleanup-worktree", () => {
    it("returns 400 when agent is running", async () => {
      ...
      const response = await request(app)
        .post(`/api/tasks/${TEST_TASK_ID}/cleanup-worktree`)
        .send({});
    ...
    it("returns 500 with recovery steps when cleanup fails", async () => {
      ...
      mocks.agentService.isAgentRunning.mockReturnValue(false);
      mocks.gitService.worktreeExists.mockResolvedValue(true);
      mocks.agentService.cleanupTaskWorktree.mockRejectedValue(
