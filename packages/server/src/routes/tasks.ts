...
} from "../services/pm-agent.service.js";
...
function mapPRCreationErrorForClient(
  errorMessage: string,
): ClientPRCreationError | null {
  const common = {
    status: 400,
 ...
 */
function getAIConfig(_req: Request): {
  ...
  if (credentials) {
    logger.debug("Using AI credentials from secrets service", {
...
router.patch(
  ...
  (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      getDataEventEmitter().emitChange({ entity: 'task', action: 'updated', id: id! });
      res.json(task);
    } catch (error) {
      next(error);
    }
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      agentService
        .cleanupTaskWorktree(id!, "task deleted")
        .catch((cleanupError) => {
      ...
      getDataEventEmitter().emitChange({ entity: 'task', action: 'deleted', id: id! });
      res.status(204).send();
    } catch (error) {
      next(error);
    }
...
/**
 * @deprecated Este endpoint ya no está activo. El flujo actual es:
 * POST /tasks/:id/start para iniciar el agente directamente.
 *
...
router.post(
  ...
  (_req: Request, res: Response) => {
    res.status(410).json({
      deprecated: true,
      message:
        "Este endpoint ya no está activo. Usá POST /tasks/:id/start para iniciar el agente directamente.",
      alternative: "/tasks/:id/start",
      migration_guide:
        "El nuevo flujo no requiere generar specs manualmente. El agente genera todo automáticamente.",
    });
  },
);
 ...
 * Valid for tasks with status: 'pending_approval'
 */
router.patch("/:id/spec", requireTaskId, (_req: Request, res: Response) => {
  res.status(410).json({
    deprecated: true,
    message:
      "Este endpoint ya no está activo. Usá POST /tasks/:id/start para iniciar el agente directamente.",
    alternative: "/tasks/:id/start",
    migration_guide:
      "El nuevo flujo no requiere specs manuales. El agente genera todo automáticamente.",
  });
});
...
router.post(
  ...
  (_req: Request, res: Response) => {
    res.status(410).json({
      ...
      alternative: "/tasks/:id/start",
      migration_guide:
        "El nuevo flujo no requiere approval de specs. El agente maneja todo automáticamente.",
    });
  },
);
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      agentService.approvePlan(id!).catch((error) => {
        logger.error("Failed to approve plan and start implementation", {
          taskId: id,
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const updatedTask = taskService.update(id!, {
        status: "planning",
        error: isRetry ? null : undefined,
      });
      if (!updatedTask) {
...
router.post(
  ...
  (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      logger.info("POST /tasks/:id/feedback", {
        id,
      ...
      if (task.status === "plan_review") {
        logger.info("Plan review chat approval", {
          id,
          message: result.data.message,
        });
        ...
        agentService.approvePlan(id!).catch((error) => {
          logger.error("Failed to approve plan from chat", {
            taskId: id,
            error: getErrorMessage(error),
          });
          const currentTask = taskService.getById(id!);
      ...
      logger.info("No active agent, resuming with user message", {
        id,
        status: task.status,
      });
    ...
    try {
      ...
      if (!agentService.isAgentRunning(id!)) {
        ...
        if (activeStatuses.includes(task.status)) {
          logger.info(
            "No active agent but task is stuck in active status, resetting",
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res.json({
        ...
        message:
          "Changes requested. Call POST /tasks/:id/execute to resume the agent.",
      });
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      if (agentService.isAgentRunning(id!)) {
        res.status(400).json({
          ...
          message:
            "Cannot clean up worktree while agent is running. Cancel the agent first.",
        });
        return;
      }
      ...
      res.json({
        status: "cleaned",
        message:
          "Worktree has been cleaned up. Next execution will start fresh.",
      });
    } catch (error) {
      next(error);
    }
    ...
    try {
      ...
      if (task.status === "done") {
        ...
        const planReviewEvent = `event: awaiting_review\ndata: ${JSON.stringify(
          {
            message:
              "Plan created. Review the plan and approve to start implementation.",
          },
        )}\n\n`;
        ...
        const pendingEvent = `event: log\ndata: ${JSON.stringify({
          ...
          level: "info",
          message:
            "Spec is ready for review. Please review and approve or edit.",
        })}\n\n`;
      ...
      const heartbeatInterval = setInterval(() => {
        try {
          ...
          logger.debug("Heartbeat write failed, clearing interval", {
            taskId: id,
            error: getErrorMessage(error),
          });
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { task } = req as RequestWithTask;
      const { id } = req.params;
      logger.debug("GET /tasks/:id/changes", {
        id,
        ...
        try {
          ...
          logger.info("Serving changes from live worktree", {
            ...
            filesWithContent: files.filter(
              (f) => f.oldContent !== undefined || f.newContent !== undefined,
            ).length,
          ...
          logger.warn(
            "Live worktree returned empty changes, trying persisted fallback",
            { taskId: id, workspacePath },
          );
          ...
          logger.warn(
            "Failed to read live worktree changes, trying persisted fallback",
            {
              taskId: id,
              workspacePath,
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      logger.info("VS Code opened for merge conflict resolution", {
        id,
        worktreePath,
      });
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const filesWithMarkers = await gitService.hasConflictMarkers(
        worktreePath,
        conflictFileList,
      );
      ...
      try {
        ...
        logger.error("Failed to complete merge commit", {
          id,
          error: errorMsg,
        });
        res.status(500).json({
      ...
      agentService.pushAndCreatePR(id!).catch((error) => {
        const errorMsg = getErrorMessage(error);
        logger.error("Failed to push and create PR after conflict resolution", {
          id,
          error: errorMsg,
        });
        taskService.update(id!, {
